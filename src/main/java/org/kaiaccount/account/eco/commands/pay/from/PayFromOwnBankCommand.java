...
public class PayFromOwnBankCommand implements ArgumentCommand {
    ...
    public @NotNull List<CommandArgument<?>> getArguments() {
        return List.of(new ExactArgument("from"), new ExactArgument("bank"), FROM, TO, PAYMENT);
    }

    @Override
    public @NotNull String getDescription() {
        return "Pay a account from your bank account";
    }
    ...
    public boolean run(CommandContext commandContext, String... strings) {
        PlayerBankAccount from = commandContext.getArgument(this, FROM);
