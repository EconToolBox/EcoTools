...
public class DateTimeArgument implements CommandArgument<LocalDateTime> {
    ...
    private final ToIntBiFunction<CommandContext, CommandArgumentContext<LocalDateTime>> maxYear;
    private final ToIntBiFunction<CommandContext, CommandArgumentContext<LocalDateTime>> minYear;

    public DateTimeArgument(String id) {
        this(id, (a, b) -> LocalDateTime.MIN.getYear(), (a, b) -> LocalDateTime.MAX.getYear());
    }

    public DateTimeArgument(String id, ToIntBiFunction<CommandContext, CommandArgumentContext<LocalDateTime>> minYear, ToIntBiFunction<CommandContext, CommandArgumentContext<LocalDateTime>> maxYear) {
        this.id = id;
    ...
    @Override
    public @NotNull Collection<String> suggest(@NotNull CommandContext commandContext, @NotNull CommandArgumentContext<LocalDateTime> commandArgumentContext) {
        String peek = commandArgumentContext.getFocusArgument();
        char splitChar = '/';
        if (peek.contains("\\")) {
            splitChar = '\\';
