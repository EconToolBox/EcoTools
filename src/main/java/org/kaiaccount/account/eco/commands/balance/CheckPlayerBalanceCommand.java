...
public class CheckPlayerBalanceCommand implements ArgumentCommand {
	...
	public static final CommandArgument<OfflinePlayer> USER = new OptionalArgument<OfflinePlayer>(PERMISSION_USER,
			(context, arg) -> {
				if (context.getSource() instanceof OfflinePlayer player) {
					return CommandArgumentResult.from(arg, 0, player);
				}
				throw new IOException("A player is needed");
			}
	);
